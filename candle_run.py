...
def run(settings):
    ...
    # PART 1: System INIT
    np.random.seed(int(settings['rng_seed']))
    print('Setting random seed ', settings['rng_seed'])
    tf.set_random_seed(int(settings['rng_seed']))
    ...
    if load_db :
        ...
        PWD = settings['pwd'] #/home/mara/CAMELYON.exps/dev05/'
        h5file = settings['h5file'] #'0109-1415/patches.hdf5'
    ...
    if training:
        ...
        #settings['epochs'] = int(math.ceil(float(settings['epochs'])/ hvd.size()))
    
        wlog('Network settings', settings)
        ...
        print('Training dataset: ', Xtrain.shape)
        ...
        model = getModel(settings, settings)

        #fitModel(model, settings, Xtrain, Ytrain, Xval, Yval, save_history_path=new_folder)
        history = fitModel(model, settings, Xtrain, Ytrain, Xval, Yval, save_history_path=new_folder)
