...
def hsv_to_rgb(hsv):
    ...
    rgb[..., 3:] = hsv[..., 3:]
    h, s, v = np.clip(hsv[..., 0],0.0,1.0), np.clip(hsv[..., 1], 0.0, 1.0), np.clip(hsv[..., 2], 0.0, 255.0)
...
class ImageDataGenerator(object):
    ...
    def random_transform(self, x, seed=None):
        ...
        if self.random_hue: #####
            if np.random.random() < 0.5:
                random_hue=np.random.uniform(-0.04,0.04)
                x = shift_hue(x,random_hue)
                
        if self.random_saturation: #####
            pass
            #x = tf.image.random_saturation(image=x, lower=0.5, upper=0.75).eval(session=sess) ######
            #with tf.Session() as sess: #####
            #    x = new_x.eval() #####
                
        if self.random_brightness: #####
            if np.random.random() < 0.5:
                x = shift_brightness(x,np.random.uniform(-0.25,0.25))
                pass
            #x = tf.image.random_brightness(image=x, max_delta=0.25).eval(session=sess) ######
            #with tf.Session() as sess: #####
            #    x = new_x.eval() #####
        
        if self.random_contrast: #####
            pass
            #x = tf.image.random_contrast(image=x, lower=0.10, upper=0.85).eval(session=sess) ######
            #with tf.Session() as sess: #####
            #        x = new_x.eval() #####
